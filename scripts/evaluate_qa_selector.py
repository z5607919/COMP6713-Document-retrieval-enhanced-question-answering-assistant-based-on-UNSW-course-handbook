...
"""Evaluate retrieval -> supervised selector -> extractive QA."""
...
def build_arg_parser():
    p = argparse.ArgumentParser(description="Evaluate supervised selector-RAG.")
    ...
    p.add_argument("--out-dir", default="data/results/qa_hybrid_selector")
    return p
...
def main():
    ...
    for i, ann in enumerate(annotations, start=1):
        ...
        reranked = selector.rerank(q, retrieved)
        selected = reranked[:1]
        answer = reader.answer_from_chunks(q, selected, top_n_contexts=args.reader_top_n)
        pred = answer.answer
        gold = ann.get("answer", "")
        n_pred = norm(pred)
        n_gold = norm(gold)
        ...
        rows.append({
            ...
            "selector_selected_chunk_id": selected_id,
            "selector_correct_top1": int(selected_id == ann.get("gold_chunk_id", "")),
    ...
    metrics = {
        ...
        "selector_top1_accuracy": sum(r["selector_correct_top1"] for r in rows) / n,
    ...
    write_csv(out_dir / "qa_selector_predictions.csv", rows, fields)
    write_csv(out_dir / "qa_selector_metrics.csv", [{"metric": k, "value": v} for k, v in metrics.items()], ["metric", "value"])
    (out_dir / "qa_selector_metrics.json").write_text(json.dumps(metrics, ensure_ascii=False, indent=2), encoding="utf-8")
    print("=" * 80)
    print("QA evaluation summary (hybrid + selector + extractive QA)")
    print("=" * 80)
    ...
    print("=" * 80)
    print(f"Saved selector QA results to: {out_dir}")
